...
// ── overlay real page copy from the DESIGN-SYSTEM extract (jvto-new-on-design-system) ──
// Enriches matched IA pages with the real seo{description,title,schema_type} + h1,
// and carries the full page copy losslessly as one `page_content` section. This is
// what makes the SSOT actually HOLD the real copy (not just {title} scaffolds).
// The extract is authored directly against SSOT routes (no jvto_dev; no URL alias).
// Scaffold is untouched where no extract row exists — no fabrication. Deterministic:
// reads a committed extract (regenerated by scripts/build-design-system-extract.mjs),
// so re-runs reproduce byte-identical output.
